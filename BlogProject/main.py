...
class Comment(db.Model):
    ...
    # connection between comments and author
    ...
    # connection between comments and blog post
    ...
    def decorated_function(*args,**kwargs):
        if current_user.is_authenticated == True:
            if current_user.id == 1:
                ...
                print("not the admin")
                flash("Only admin has access to this route.")
...
def register():
    ...
    if register_form.validate_on_submit():
        ...
        print(email_exists)
            # if email doesnt exist the registration proccess continues
        if email_exists:
            flash("An account using this email already exists.Please try to log in.")
            return redirect(url_for("login"))
        else:
            print("email doesnt exist")
            password = register_form.Password.data

            # Uses Werkzeug to hash the user's password when creating a new user.
...
def login():
    ...
    if login_form.validate_on_submit():
        users_email = login_form.Email.data
        email_exists = db.session.query(
            db.session.query(User).filter_by(email=users_email).exists()
        ).scalar()
         # if the email provided belongs to a user and the password provided is correct then the user logs in successfully

        if email_exists:
            the_user = db.session.execute(db.select(User).where(User.email == users_email)).scalar()
            print(the_user.is_authenticated)
            right_password = the_user.password
            if check_password_hash(pwhash=right_password,password=login_form.Password.data):
                login_user(the_user)
...
# starting page that shows all available posts
...
# Shows selected post and allows logged-in users to comment on posts
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            comment_body = form.comment_editor.data
            comment_author = current_user
            new_comment = Comment(content=comment_body,comment_author=comment_author,post_id=post_id)
            db.session.add(new_comment)
            db.session.commit()
            return redirect(url_for("show_post",post_id=post_id))
        else:
            flash("Commenting is only available to logged in users.If you want to upload a comment, please log in!")
            return redirect(url_for("login"))
